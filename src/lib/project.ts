...
export function getAllProjectCaseStudies(): ProjectCaseStudyPreview[] {
    ...
    const caseStudies = slugs
        ...
        .filter(
            (caseStudy): caseStudy is ProjectCaseStudyPreview =>
                caseStudy !== null,
        )
        .sort((a, b) => {
            // Sort by featuredf first, then by title
...
// Get all unique technologies from public case studies
    ...
    const caseStudiesWithScore = allCaseStudies
        ...
        .map((caseStudy) => {
            const sharedTechnologies =
                caseStudy.frontmatter.technologies.filter((tech) =>
